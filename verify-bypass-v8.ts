...
async function runTest() {
  console.log("🔍 Iniciando verificação FINAL da lógica de bypass (V8)...");

  const traceId = "verify-v8";
  ...
  // Inserir com status READY para o worker capturar
  ...
  // Passamos o objeto com os mesmos dados do DB
  await processSingleFollowup(syntheticJob, traceId);

  // Verificar se o job foi cancelado
  const { data: updatedSynthetic, error: errS } = await supabaseAdmin
    .from("crm_followups")
  ...
  if (errS) {
    console.error("Erro no select V8:", errS);
    process.exit(1);
  }

  console.log("Status final:", updatedSynthetic?.status);
  console.log("Motivo final:", updatedSynthetic?.cancel_reason);
  ...
  // 2. Testar job real que contém "Teste" no trigger mas reason real
  ...
  console.log("\n✨ Verificação V8 concluída com sucesso!");
}
