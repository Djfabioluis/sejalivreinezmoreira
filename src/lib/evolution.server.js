...
import { enforceNoCpfInSubscriptionFlow, containsCpfSolicitation, PHONE_REQUEST_MESSAGE } from "./subscription-policy.server";
...
export async function setWebhook(instance, webhookUrl) {
...
export async function getQrCode(instance) {
    ...
    if (!res.ok)
        return null;
    const b64 = res.data?.base64 ?? res.data?.qrcode?.base64;
    if (!b64)
        return null;
...
export async function sendEvolutionText(instance, to, body, typingMs = 0) {
...
export async function sendEvolutionPresence(instance, to, presence = "composing", delayMs = 3000) {
...
export async function sendEvolutionAudio(instance, to, mp3) {
    const number = to.replace(/\D/g, "");
...
export async function fetchEvolutionMediaBase64(instance, messageKeyId) {
...
export function instanceNameFor(phoneDigits) {
