...
import { resolveOutboundInstanceForUnit } from "./outbound-resolver.server";
 ...
 */
export function ensureAIAllowedToReply(conv) {
    if (!conv) {
    ...
    if (conv.attendance_mode === "HUMAN")
        return { allowed: false, reason: "ATTENDANCE_MODE_HUMAN" };
    if (conv.human_takeover_detected === true)
        return { allowed: false, reason: "HUMAN_TAKEOVER_DETECTED" };
    if (conv.ai_paused_at)
        return { allowed: false, reason: conv.ai_pause_reason || "AI_PAUSED" };
...
export async function replyToUser(params) {
    ...
    if (!params.allowDuringHumanMode) {
        const gate = ensureAIAllowedToReply(conv);
        if (!gate.allowed) {
    ...
    await sendEvolutionPresence(params.instance, params.phone, "composing", typingMs).catch(() => false);
