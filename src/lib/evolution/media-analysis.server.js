...
const IMAGE_PROMPT = "Descreva objetivamente, em português, o que aparece nesta imagem, focando no que é útil para um salão de beleza (cabelo, unhas, maquiagem, comprovantes, prints de agendamento). Máximo 2 frases. NÃO transcreva nem descreva dados sensíveis como CPF completo, número de cartão, senha ou dados bancários — apenas indique que existe um documento, se for o caso.";
const VIDEO_PROMPT = "Resuma objetivamente, em português, o conteúdo deste vídeo curto: o que é mostrado visualmente e o que é falado. Máximo 3 frases. Não descreva dados sensíveis (documentos, cartões, senhas).";
const DOC_PROMPT = "Resuma em português o conteúdo relevante deste documento para atendimento de um salão de beleza. Máximo 3 frases. Não reproduza dados sensíveis completos (CPF, cartão, dados bancários) — mencione apenas que existem, se houver.";
function requireKey() {
...
async function callVision(parts) {
...
export async function analyzeMediaForConversation(params) {
    ...
    try {
        if (params.sourceType === "audio") {
            ...
            if (!transcription)
                return { ...base, warnings: ["empty_transcription"] };
        ...
        if (params.sourceType === "image") {
            ...
            if (!description)
                return { ...base, warnings: ["empty_description"] };
            return {
                ...base,
                success: true,
                visualDescription: description,
                confidence: 0.85,
                extractedText: `[Imagem enviada pelo cliente]\nDescrição: ${description}` +
        ...
        if (params.sourceType === "video") {
            ...
            if (!summary)
                return { ...base, warnings: ["empty_video_summary"] };
            return {
                ...base,
                success: true,
                visualDescription: summary,
                confidence: 0.75,
                extractedText: `[Vídeo enviado pelo cliente]\nResumo: ${summary}` +
                    (params.caption ? `\nLegenda: ${params.caption}` : ""),
            };
        }
        // Documento
        ...
        if (!summary)
            return { ...base, warnings: ["empty_document_summary"] };
        return {
            ...base,
            success: true,
            confidence: 0.8,
            extractedText: `[Documento enviado pelo cliente]\nResumo: ${summary}` +
                (params.caption ? `\nLegenda: ${params.caption}` : ""),
        };
    }
    catch (error) {
