...
import { ALLOWED_MIME, BLOCKED_EXTENSIONS, MEDIA_LIMITS, } from "./media-types";
/** Valida metadados ANTES de baixar qualquer byte. */
export function validateMediaMetadata(msg) {
...
export function storeTemporaryMedia(key, media) {
    tempStore.set(key, media);
}
export function getTemporaryMedia(key) {
    return tempStore.get(key);
}
export function deleteTemporaryMedia(key) {
