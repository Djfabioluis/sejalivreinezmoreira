import type { NormalizedIncomingMessage, MediaSourceType } from "./media-types";
...
export function unwrapMessage(message: any): { content: any; isViewOnce: boolean } {
  let content = message;
...
export function normalizeIncomingMessage(
  ...
  const pick = (
    node: any,
    messageType: Exclude<MediaSourceType, "text">,
  ): NormalizedIncomingMessage => ({
    ...base,
    messageType,
  ...
  if (content?.audioMessage) return pick(content.audioMessage, "audio");
  if (content?.imageMessage) return pick(content.imageMessage, "image");
  if (content?.videoMessage) return pick(content.videoMessage, "video");
  if (content?.stickerMessage) return pick(content.stickerMessage, "image");
  if (content?.documentMessage) return pick(content.documentMessage, "document");
  if (content?.ptvMessage) return pick(content.ptvMessage, "video");
...
export function hasProcessableContent(msg: NormalizedIncomingMessage): boolean {
