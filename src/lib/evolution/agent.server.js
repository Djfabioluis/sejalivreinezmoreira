...
export async function runAgentFlow(msg, textOverride) {
    const messageId = msg.messageId;
    const instance = msg.instance;
    const trace = msg._trace;
    const traceId = trace?.getTraceId() || msg._traceId || `${instance}:${messageId}`;
    ...
    try {
        ...
        const isHumanMode = conv?.instance === instance && (conv?.attendance_mode === "HUMAN" ||
        ...
        if (isHumanMode) {
            ...
            const stillPaused = customerRequested || takeoverAt === 0 || minutesSinceTakeover < HUMAN_TAKEOVER_TIMEOUT_MINUTES;
            if (stillPaused) {
        ...
        if (text) {
            const { detectHumanTakeoverIntent, HUMAN_TRANSFER_MESSAGE, AI_PAUSE_REASON_CUSTOMER } = await import("@/lib/human-takeover");
        ...
        const history = normalizeConversationHistory(conv?.messages || [], text, messageId);
        const { extractBookingSlots, mergeBookingContext, detectSubscriptionIntent, nextRequiredSlot, } = await import("@/lib/booking/context");
        const { patchCustomerContext } = await import("@/lib/chat.server");
        const customerContext = conv?.customer_context || {};
        ...
        if (agent.unidade_id)
            bookingContext.unitId = String(agent.unidade_id);
