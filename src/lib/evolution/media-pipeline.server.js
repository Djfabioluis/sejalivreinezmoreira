...
import { MEDIA_FALLBACK_TEXT, MEDIA_PLACEHOLDER, OVERSIZE_REPLY, } from "./media-types";
import { downloadEvolutionMedia, validateMediaMetadata, storeTemporaryMedia, deleteTemporaryMedia, } from "./media.server";
import { analyzeMediaForConversation } from "./media-analysis.server";
export function mediaPlaceholderText(msg) {
    if (msg.messageType === "text")
        return msg.text;
    const base = MEDIA_PLACEHOLDER[msg.messageType];
    return msg.caption ? `${base}\n“${msg.caption}”` : base;
}
async function updateMessage(params) {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    await supabaseAdmin.rpc("update_wa_message_metadata", {
...
export async function processIncomingMedia(params) {
    ...
    const kind = normalized.messageType;
    ...
    if (!validation.ok) {
        ...
        const rejectText = validation.reason === "size" || validation.reason === "duration"
            ? OVERSIZE_REPLY
    ...
    const { data: claimed } = await supabaseAdmin.rpc("evo_claim_media", {
        p_instance: instance,
    ...
    const finish = async (status, error) => {
        await supabaseAdmin.rpc("evo_finish_media", {
            p_instance: instance,
            p_message_id: messageId,
            p_media_hash: hash,
            p_status: status,
    ...
    try {
        ...
        const completedEvent = kind === "audio"
