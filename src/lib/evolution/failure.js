...
export const GENERIC_FALLBACK_TEXT = "Não consegui concluir sua solicitação agora, mas já encaminhei para nossa equipe humana te ajudar em instantes! 💜";
...
export function sanitizeErrorText(input, maxLen = 1000) {
...
export function classifyFailure(err) {
    ...
    if (status === 402 || /payment required|insufficient credit|credits? exhausted/.test(msg)) {
        return {
            ...
            userMessage: "Nosso atendimento automático está temporariamente indisponível 😔\n\nJá avisei nossa equipe e em instantes alguém te responde por aqui 💛",
            expected: true,
            escalate: true,
        };
    }
    if (status === 429 || /rate limit|too many requests/.test(msg)) {
        return {
            ...
            userMessage: "Estou recebendo muitas mensagens ao mesmo tempo agora 😅\n\nPode me mandar de novo em alguns segundinhos? 💛",
            expected: true,
            escalate: false,
        };
    }
    ...
    const known = {
        ...
        no_assigned_professionals: "No momento não temos profissional disponível para esse serviço nesta unidade 😔 Posso te mostrar outras opções?",
        professional_not_assigned_to_service: "Essa profissional não realiza esse serviço nesta unidade 😊 Quer que eu mostre quem atende?",
