...
export function normalizeBempBooking(raw: any): BempBooking | null {
...
export function filterFutureBookingsForUnit(
  rawList: unknown,
  ...
  return list
    ...
    .filter((b): b is BempBooking => b !== null)
    .filter((b) => {
      ...
      if (unit && b.unitId && b.unitId !== unit) return false;
      return true;
    })
...
export function buildSingleCancelConfirmation(b: BempBooking): string {
  return [
...
export function buildMultipleCancelList(list: BempBooking[]): string {
  const lines = list.map(
    (b, i) =>
      `${i + 1}. ${b.serviceName} — ${formatBookingDate(b.start).slice(0, 5)} às ${formatBookingTime(b.start)}`,
  );
  return ["Encontrei estes agendamentos 💜", ...lines, "", "Qual deles você deseja cancelar?"].join("\n");
}

export function buildCancelSuccessMessage(b: BempBooking): string {
...
export const NO_FUTURE_BOOKINGS_MESSAGE =
  "Não encontrei agendamentos futuros para cancelar. 💜";
export const CANCEL_FAILED_MESSAGE =
  "Não consegui concluir o cancelamento agora. 💜\nVou precisar tentar novamente.";

/** Escolhe um booking a partir da resposta do cliente ("1", "2", ou nome do serviço). */
export function chooseBookingFromReply(
  text: string,
  options: BempBooking[],
): BempBooking | null {
  const t = String(text || "")
...
const CANCEL_CONFIRM =
  /^(sim|s|isso|confirmo|confirmar|pode|pode cancelar|cancela|cancelar|quero|quero sim|isso mesmo|ok|claro|com certeza)[\s.,!?💜]*$/;
...
export function isCancelAbort(text: string): boolean {
