...
/* Subscription intent                                                 */
/* ------------------------------------------------------------------ */
const SUBSCRIPTION_INTENT_PATTERNS = [
    /\bplanos?\b/i,
    /\bmeu\s+plano\b/i,
    /\bassinaturas?\b/i,
    /\bassinante\b/i,
...
export function detectSubscriptionIntent(text) {
    if (!text)
        return false;
...
export function detectHarmonizationIntent(text) {
    if (!text)
        return false;
...
export function extractBookingSlots(text, now = new Date(), previous = null) {
    const out = {};
    if (!text)
        return out;
    const t = text.trim();
    ...
    if (previous?.clarificationRequired && previous.candidates?.length) {
        ...
        if (ordinalMatch) {
            ...
            if (word.startsWith("prim"))
                index = 0;
            else if (word.startsWith("segu"))
                index = 1;
            else if (word.startsWith("terc"))
                index = 2;
            else if (word.startsWith("quar"))
                index = 3;
            else if (word.startsWith("quin"))
                index = 4;
        }
        ...
        const exactMatch = previous.candidates.find((c) => normalizedInput.includes(c.name.toLowerCase()) ||
    ...
    if (detectSubscriptionIntent(t))
        out.subscriptionIntent = true;
...
/* Merge                                                               */
/* ------------------------------------------------------------------ */
const EMPTY = (v) => v === undefined || v === null || v === "" || v === "UNKNOWN";
/**
 ...
 */
export function newBookingSessionId() {
...
export function startNewBookingSession(ctx) {
    const next = { ...ctx };
...
/* Profissional                                                        */
/* ------------------------------------------------------------------ */
const ANY_PROFESSIONAL = /(qualquer|tanto\s*faz|indiferente|sem\s*prefer[eê]ncia|quem\s*(estiver|tiver)\s*dispon[ií]vel|o\s*que\s*estiver\s*dispon[ií]vel|pode\s*ser\s*qualquer)/i;
/** TRUE quando o cliente aceita qualquer profissional disponível. */
export function isAnyProfessionalChoice(text) {
    if (!text)
        return false;
 ...
 */
export function matchProfessionalChoice(text, options) {
    ...
    if (ordIdx >= 0 && ordIdx < options.length)
        return options[ordIdx];
...
export function isAnyProfessionalIndex(text, options) {
    if (!text || !options)
        return false;
...
export function hasCurrentSessionPeriod(ctx) {
    if (!ctx.period)
        return false;
    if (!ctx.bookingSessionId)
        return false;
    return ctx.periodSessionId === ctx.bookingSessionId;
}
export function mergeBookingContext(previous, extracted) {
    const prev = previous || {};
    let next = { ...prev };
    const isReset = extracted?._isReset === true;
    ...
    const previousService = prev.serviceId
        ...
        : prev.serviceText
            ...
            : null;
    const startsNewSession = isReset ||
    ...
    if (!next.serviceId && prev.serviceId && extracted?.serviceId === undefined && !isReset)
        next.serviceId = prev.serviceId;
    if (!next.serviceName && prev.serviceName && extracted?.serviceName === undefined && !isReset)
        next.serviceName = prev.serviceName;
    if (!next.serviceText && prev.serviceText && extracted?.serviceText === undefined && !isReset)
        next.serviceText = prev.serviceText;
    if (!next.date && prev.date && extracted?.date === undefined && !isReset)
        next.date = prev.date;
    return next;
}
/* ------------------------------------------------------------------ */
...
export function buildBookingContextBlock(ctx) {
...
/* Proteção de saída                                                   */
/* ------------------------------------------------------------------ */
const SUBSCRIPTION_OUTPUT_PATTERNS = [
    /\bassinaturas?\b/i,
    /\bassinante\b/i,
    /\bplanos?\b/i,
    /\bbenef[ií]cios?\b/i,
 ...
 */
export function enforceNoSubscriptionFlow(text, ctx) {
    if (!text)
        return { text, blocked: false };
    if (ctx.subscriptionIntent === true)
        return { text, blocked: false };
...
export function fallbackQuestionFor(ctx) {
...
/* ------------------------------------------------------------------ */
const GREETING_ONLY = /^(oi+|ol[áa]|ola|opa|e a[íi]|bom\s*dia|boa\s*tarde|boa\s*noite|hey|hi|hello|oi\s*tudo\s*bem\??|ol[áa]\s*tudo\s*bem\??|tudo\s*bem\??)[\s,.!?😊💜🙏👋]*$/i;
 ...
 */
export function clearTransientBooking(ctx) {
    const next = startNewBookingSession({ ...ctx });
    next.serviceId = null;
 ...
 */
export function ensureNoDuplicateBookingQuestion(text, ctx) {
...
/* ------------------------------------------------------------------ */
const CANCEL_INTENT = /^(cancelar|cancela|cancelamento|desmarcar|desmarca|quero\s+desmarcar|quero\s+cancelar|cancelar\s+meu\s+horario|cancelar\s+agendamento|cancelar\s+o\s+agendamento|cancelar\s+meu\s+agendamento|desistir|desisti|pare|parar|nao\s+quero\s+mais|nao\s+quero|deixa\s+pra\s+la)[\s.,!?💜]*$/;
...
export function hasBookingInProgress(ctx) {
    return Boolean(ctx.serviceId ||
        ctx.serviceName ||
        ctx.serviceText ||
        ctx.date ||
        ctx.period ||
...
export function resetBookingForCancel(ctx) {
    ...
    next.createBookingKey = null;
    next.confirmationSentFor = null;
    if (next.appointmentStatus !== "CONFIRMED")
        next.appointmentStatus = "NONE";
    return next;
}
export const CANCEL_FLOW_MESSAGE = "Sem problema 💜 O agendamento em andamento foi cancelado. Se quiser começar novamente, é só me chamar.";
export const CANCEL_IDLE_MESSAGE = "Sem problema 💜 Não há nenhum agendamento em andamento. Se quiser iniciar um novo, é só me chamar.";
