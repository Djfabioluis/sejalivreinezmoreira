...
import { detectCancelIntent, hasBookingInProgress, resetBookingForCancel, CANCEL_FLOW_MESSAGE, } from "./context";
import { buildCancelSuccessMessage, buildMultipleCancelList, buildSingleCancelConfirmation, chooseBookingFromReply, filterFutureBookingsForUnit, isCancelAbort, isCancelConfirmation, CANCEL_FAILED_MESSAGE, NO_FUTURE_BOOKINGS_MESSAGE, } from "./cancel-existing";
...
async function runCancel(booking, conversationUnitId, ctx, deps, t) {
...
export async function handleCancelFlow(params) {
    ...
    if (anyCtx.pendingCancellation === true) {
        const options = Array.isArray(anyCtx.pendingCancellationOptions)
            ? anyCtx.pendingCancellationOptions
            : [];
        const selectedId = anyCtx.pendingCancellationBookingId ?? null;
        ...
        if (!selected && options.length > 0) {
            ...
            if (chosen) {
                const next = { ...ctx, pendingCancellationBookingId: chosen.id };
