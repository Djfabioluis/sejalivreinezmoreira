...
export function getLocalBookingDate(now: Date = new Date(), tz: string = SALON_TZ): string {
...
export function resolveRelativeDate(
  ...
  if (/depois\s+de\s+amanha\b/.test(t)) return addLocalDays(today, 2);
  if (/\bamanha\b/.test(t)) return addLocalDays(today, 1);
  return null;
}
