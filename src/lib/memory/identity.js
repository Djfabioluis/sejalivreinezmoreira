...
export function normalizeMemoryPhone(raw) {
    if (!raw)
        return "";
...
export function phoneFromConversationKey(conversationKey) {
    if (!conversationKey)
        return "";
...
export function resolveOrgKey(instanceOrKey) {
...
export function maskSensitive(text) {
    ...
    for (const pattern of SENSITIVE_PATTERNS)
        out = out.replace(pattern, "[dado protegido]");
    return out;
}
...
export function isForbiddenMemoryValue(value) {
