...
function listOrDash(list: unknown, max = 6): string {
  if (!Array.isArray(list) || list.length === 0) return "não registrado";
  return list.slice(0, max).map((v) => String(v)).join(", ");
}

function objectOrDash(value: unknown): string {
  if (!value || typeof value !== "object" || Array.isArray(value)) return "não registrado";
  const entries = Object.entries(value as Record<string, unknown>).filter(([, v]) => v !== null && v !== "");
  if (entries.length === 0) return "não registrado";
 ...
 */
export function buildMemoryPromptBlock(memory: Partial<CustomerMemoryRow> | null): string {
  if (!memory) return "";
  ...
  if (lowConfidence) {
    block.push(
      "- ATENÇÃO: a memória deste cliente tem confiança baixa. Trate os itens acima como possibilidades a confirmar, nunca como fatos.",
    );
  }
