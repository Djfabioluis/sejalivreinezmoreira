...
function mergeNote(value, note) {
    if (typeof value !== "string" || value.trim().length === 0)
        return note;
    return value.includes(note) ? value : `${value.trim()}\n${note}`;
}
export function withProfessionalPreferenceNote(input) {
    const payload = { ...input };
    const hasProfessionalPreference = payload.professional_id != null ||
        payload.professionalId != null ||
        payload.preferred_professional_id != null;
    if (!hasProfessionalPreference)
        return payload;
...
export function extractBempAppointmentId(result) {
    if (!result || typeof result !== "object" || Array.isArray(result))
        return null;
    const obj = result;
    const direct = readStringId(obj.id) ??
    ...
    for (const key of ["data", "schedule", "appointment", "result"]) {
        ...
        if (nested && typeof nested === "object" && !Array.isArray(nested)) {
            const nestedId = extractBempAppointmentId(nested);
            if (nestedId)
                return nestedId;
        }
    }
...
export async function tryUpdateBempScheduleNote(result, note) {
...
async function readSettingsFromDb() {
    ...
    try {
        ...
        const raw = data.conteudo;
        const parsed = JSON.parse(raw);
        const value = parsed.dominio && parsed.token
...
export async function getBempConfig() {
    ...
    if (!dominio || !token) {
        throw new Error("Credenciais Bemp não configuradas. Acesse Configuração → Integração Bemp para inserir domínio e token.");
    }
    return {
        dominio,
        token,
...
export async function getBempSettingsSafe() {
    const db = await readSettingsFromDb();
    if (db)
        return { dominio: db.dominio, hasToken: !!db.token, source: "db" };
    const envDom = process.env.BEMP_DOMINIO;
    const envTok = process.env.BEMP_TOKEN;
    if (envDom && envTok)
        return { dominio: envDom, hasToken: true, source: "env" };
...
export class BempHttpError extends Error {
    ...
    endpoint;
    details;
    code = "bemp_http_error";
    constructor(status, endpoint, message, details) {
        super(message);
        this.status = status;
...
export async function bempFetch(url, init) {
    const cfg = await getBempConfig();
    ...
    try {
        ...
        console.error(`[bemp] bemp_request_failed: method=${method}, endpoint=${endpoint}, durationMs=${Date.now() - startedAt}, error=${networkErr instanceof Error ? networkErr.message : String(networkErr)}`);
        throw networkErr;
    }
    ...
    if (!res.ok) {
        const msg = body && typeof body === "object" && !Array.isArray(body) && "message" in body
            ? String(body.message)
        ...
        const safeBody = (typeof text === "string" ? text : "")
            ...
            .slice(0, 400);
        console.error(`[bemp] bemp_response_error: method=${method}, endpoint=${endpoint}, status=${res.status}, durationMs=${durationMs}, body=${safeBody}`);
        throw new BempHttpError(res.status, endpoint, `Bemp ${res.status}: ${msg}`, safeBody);
    }
    console.log(`[bemp] bemp_response_ok: method=${method}, endpoint=${endpoint}, status=${res.status}, durationMs=${durationMs}`);
    return body;
}
