...
import { buildBookingContextBlock, } from "@/lib/booking/context";
...
export function runTool(label, fn, ctx = {}) {
    ...
    return fn()
        ...
        if (failure.escalate && ctx.conversationKey) {
            try {
                ...
                }).eq("phone", ctx.conversationKey.split(':')[1] || ctx.conversationKey);
            }
            catch (handoffErr) {
...
export function safeTool(label, fn, ctx = {}) {
    return runTool(label, fn, ctx);
}
export async function resolveEffectiveUnit(params) {
...
export async function patchCustomerContext(conversationKey, patch) {
    if (!conversationKey)
        return;
    try {
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export function replacePromptVariables(prompt, vars) {
    let res = prompt;
...
function buildTools(sandbox, fallbackAgentUnitId, conversationKey, subscriptionIntent, traceId, bookingContext = null) {
    const safeToolLocal = (label, fn) => runTool(label, fn, { conversationKey, effectiveUnitId: fallbackAgentUnitId });
    return {
        list_services: tool({
        ...
        list_slots: tool({
            ...
            execute: async (input) => safeToolLocal("list_slots", async () => {
        ...
        create_appointment: tool({
            ...
            execute: async (input) => safeToolLocal("create_appointment", async () => {
                if (sandbox)
                    return { success: true, sandbox: true, simulated: true, id: `SIM-${Date.now()}` };
                return BempService.createAppointment(input);
            }),
        }),
    };
}
export async function runAgentWithLogging(opts) {
...
export async function runAgent(opts) {
    ...
    const text = opts.text || rawMessages[rawMessages.length - 1]?.content || "";
    ...
    const provider = createLovableAiGatewayProvider(gatewayKey);
    const model = provider("google/gemini-2.5-flash");
    let bookingContext = opts.bookingContext ||
    ...
    if (bookingContext.clarificationRequired && !bookingContext.serviceId) {
        const matchedCandidate = bookingContext.candidates?.find(c => normalizeServiceSearchText(text).includes(normalizeServiceSearchText(c.name)));
        if (matchedCandidate) {
            bookingContext = mergeBookingContext(bookingContext, {
