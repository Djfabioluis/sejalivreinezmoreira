...
// import { type AgentOptions } from "./agent-types"; // Comentado para usar a interface local exportada abaixo e evitar conflito
import { z } from "zod";
...
function runTool(label, fn, ctx = {}) {
    ...
    return fn()
        ...
        console.error(`[chat] tool_failed: ${base}, executionTimeMs=${Date.now() - startedAt}, code=${failure.code}, name=${info.name}, message=${info.message}, details=${sanitizeErrorText(err?.details ?? "", 300)}`);
        if (info.stack)
            console.error(`[chat] tool_failed_stack: tool=${label}\n${info.stack}`);
        return {
            success: false,
            code: failure.code,
...
function buildTools(sandbox, fallbackAgentUnitId, conversationKey) {
    ...
    const base = {
        ...
        create_appointment: tool({
            ...
            execute: async (input) => safeTool("create_appointment", async () => {
                ...
                const { getAvailableServiceAssignments, validateProfessionalServiceAssignment } = await import("@/lib/bemp/assignments.server");
                ...
                if (sandbox) {
                    return {
                        ...
                        id: `SIM-${Date.now()}`,
                        status: "simulated",
                        message: "Agendamento SIMULADO (modo sandbox). Nada foi gravado na Bemp. (Confirmação por WhatsApp não é enviada em sandbox.)",
                ...
                try {
                    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                    const { sendWhatsAppText, formatBrDateTime } = await import("@/lib/whatsapp-send.server");
                    ...
                    try {
                        const cfg = await getBempConfig();
                        const services = (await bempFetch(`${cfg.apiBase}/salons/${effectiveUnitId}/services`));
        ...
        cancel_appointment: tool({
            description: "Cancela um agendamento existente na Bemp. Só chame após confirmação explícita do cliente sobre qual agendamento cancelar.",
            inputSchema: z.object({
        ...
        reschedule_appointment: tool({
            ...
            execute: async (input) => safeTool("reschedule_appointment", async () => {
                ...
                try {
                    ...
                    try {
                        const cfg = await getBempConfig();
                        const services = (await bempFetch(`${cfg.apiBase}/salons/${input.salon_id}/services`));
                        if (Array.isArray(services)) {
                            const found = services.find((s) => Number(s.id) === input.service_id);
                            if (found && typeof found.name === "string")
                                serviceName = found.name;
