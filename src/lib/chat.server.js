...
import { convertToModelMessages, streamText, generateText, stepCountIs, tool } from "ai";
import { z } from "zod";
...
import { bempFetch, getBempConfig, BEMP_WEBHOOK_BASE, PROFESSIONAL_PREFERENCE_NOTE, extractBempAppointmentId, tryUpdateBempScheduleNote, withProfessionalPreferenceNote, } from "@/lib/bemp.server";
...
function runTool(label, fn, ctx = {}) {
    ...
    return fn()
        ...
        if (ctx.conversationKey) {
            ...
            if (stage) {
                await updateCustomerPipeline({
                    ...
                    stage,
                    abandonmentReason: result?.abandon_trigger
                });
            }
        }
        return result;
        ...
        console.error(`[chat] tool_failed: ${base}, durationMs=${durationMs}, code=${failure.code}, integration=${failure.code.startsWith('bemp') ? 'BEMP' : 'INTERNAL'}, statusCode=${info.status}, message=${info.message}`);
...
function safeTool(label, fn, ctx = {}) {
    return runTool(label, fn, ctx);
}
async function resolveEffectiveUnit(params) {
    const { conversationKey, agentUnitId } = params;
    let conversationUnitId = null;
    let conversationUnitName = null;
    ...
    if (effectiveUnitId) {
        try {
            ...
            const salons = (await bempFetch(`${cfg.apiBase}/salons`));
            const list = Array.isArray(salons) ? salons : (salons?.data ?? salons?.salons ?? []);
            const found = list.find((s) => String(s?.id) === String(effectiveUnitId));
    ...
    return {
        ...
        source: conversationUnitId ? "conversation" : "agent",
        conversationUnitId,
        agentUnitId,
    };
}
/** Mescla campos no customer_context da conversa (no-op se não houver conversationKey). */
async function patchCustomerContext(conversationKey, patch) {
    if (!conversationKey)
        return;
    try {
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
        const { data } = await supabaseAdmin
            ...
            .eq("phone", conversationKey)
            .maybeSingle();
        const current = data?.customer_context || {};
        const { error } = await supabaseAdmin
            .from("wa_conversas")
            .update({ customer_context: { ...current, ...patch } })
            .eq("phone", conversationKey);
        if (error)
            console.error(`[chat] context_patch_failed: ${error.message}`);
    }
    catch (e) {
 ...
 */
export function subscriptionContextLine(ctx) {
...
async function resolveServiceForEffectiveUnit(params) {
    console.log(`[chat] service_resolution_started: serviceName="${params.serviceName}", unitId=${params.effectiveUnitId}`);
    const cfg = await getBempConfig();
    const services = (await bempFetch(`${cfg.apiBase}/salons/${params.effectiveUnitId}/services`));
    const list = Array.isArray(services) ? services : services?.data ?? services?.services ?? [];
    ...
    let found = list.find((s) => normalize(s?.name || s?.nome || "") === target);
...
function buildTools(sandbox, fallbackAgentUnitId, conversationKey, currentMessageId) {
    // Sombreia o helper do módulo injetando o contexto da conversa em todos os logs de tool.
    const safeTool = (label, fn) => runTool(label, fn, { conversationKey, effectiveUnitId: fallbackAgentUnitId });
    const base = {
        transfer_conversation_unit: tool({
            description: "Transfere REALMENTE a conversa para outra unidade operacional. Use somente APÓS o cliente confirmar claramente que deseja ser atendido em outra unidade (ex.: após ele dizer 'Sim' para a sua pergunta de confirmação). Não use para simples perguntas informativas.",
            ...
            execute: async ({ target_unit_id, reason, confirmed }) => safeTool("transfer_conversation_unit", async () => {
                if (!conversationKey)
                    throw new Error("Chave da conversa não fornecida para transferência.");
                if (!confirmed) {
                ...
                const previousContext = (conv.customer_context ?? {});
                ...
                try {
                    ...
                    if (conv.unidade_id)
                        invalidateAssignmentsCache(conv.unidade_id);
                ...
                try {
                    ...
                    const salons = (await bempFetch(`${cfg.apiBase}/salons`));
                    const list = Array.isArray(salons) ? salons : (salons?.data ?? salons?.salons ?? []);
                    const found = list.find((s) => String(s?.id) === String(target_unit_id));
                    if (found?.name || found?.nome) {
        ...
        list_units_info: tool({
            description: "Use quando o cliente perguntar quantas unidades existem, quais são as unidades/lojas/endereços. Retorna as unidades ativas na Bemp com endereço. NÃO use para trocar a unidade do agendamento automaticamente.",
            inputSchema: z.object({}),
            execute: async () => safeTool("list_units_info", async () => {
                const cfg = await getBempConfig();
                const raw = await bempFetch(`${cfg.apiBase}/salons`);
                const arr = Array.isArray(raw) ? raw : (raw?.data ?? raw?.salons ?? []);
                const active = arr.filter((s) => {
                    const flag = s?.active ?? s?.ativo ?? s?.is_active ?? s?.status;
                    if (flag === undefined || flag === null)
                        return true;
                    if (typeof flag === "string")
                        return !/inativ|disabled|false|0/i.test(flag);
                    return Boolean(flag);
                });
        ...
        list_salons: tool({
            ...
            inputSchema: z.object({}),
            execute: async () => safeTool("list_salons", async () => {
                const cfg = await getBempConfig();
                return await bempFetch(`${cfg.apiBase}/salons`);
            }),
        }),
        search_services: tool({
            description: "Busca serviços disponíveis na unidade por nome ou categoria (ex: MECHAS). Retorna uma lista de opções reais do BEMP. Use quando o cliente perguntar genericamente por um tipo de serviço ou serviço específico.",
            inputSchema: z.object({
            ...
            execute: async ({ query, category }) => safeTool("search_services", async () => {
                ...
                if (searchCategory) {
                    ...
                    if (result.success && result.data) {
                        ...
                        const promosWithService = await Promise.all(activePromotions.map(async (p) => {
        ...
        list_services: tool({
            description: "Lista SOMENTE os serviços que possuem ao menos um profissional atribuído na unidade efetiva (fonte: BEMP). Nunca apresente serviços fora deste retorno.",
            inputSchema: z.object({ salon_id: z.number().optional() }),
            execute: async () => safeTool("list_services", async () => {
        ...
        list_professionals: tool({
            description: "Lista SOMENTE os profissionais realmente atribuídos ao serviço na unidade efetiva (fonte: BEMP). Informe o nome do serviço.",
            inputSchema: z.object({
            ...
            execute: async ({ service_name }) => safeTool("list_professionals", async () => {
                const { effectiveUnitId, effectiveUnitName } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                if (!effectiveUnitId)
                    return { success: false, code: "unit_not_resolved", message: "Não foi possível identificar a unidade correta." };
                console.log(`[chat] effective_unit_for_assignments: ${effectiveUnitId}`);
                const { resolveServiceAssignment, getProfessionalsForService, computeProfessionalSelection } = await import("@/lib/bemp/assignments.server");
                ...
                const { professionals, professionalsCount, autoSelectProfessional, autoSelect, askPreference, includeNoPreference, selectedProfessional, } = selection;
                ...
                if (selectedProfessional) {
                    ...
                    console.log(`[chat] single_professional_auto_selected: unit=${effectiveUnitId}, service=${service.id}, professional=${selectedProfessional.id}`);
                }
                return {
                    success: true,
                    effectiveUnitId,
        ...
        list_services_for_professional: tool({
            description: "Quando o cliente escolher primeiro um profissional, lista SOMENTE os serviços atribuídos a esse profissional na unidade efetiva (fonte: BEMP).",
            inputSchema: z.object({ professional_name: z.string() }),
            execute: async ({ professional_name }) => safeTool("list_services_for_professional", async () => {
                const { effectiveUnitId } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                if (!effectiveUnitId)
                    return { success: false, code: "unit_not_resolved", message: "Não foi possível identificar a unidade correta." };
                const { resolveProfessionalByName, getServicesForProfessional } = await import("@/lib/bemp/assignments.server");
        ...
        list_slots: tool({
            description: "Lista horários disponíveis. Passe professional_id apenas se o cliente escolheu um profissional específico. A combinação unidade+serviço+profissional é validada no BEMP.",
            inputSchema: z.object({
            ...
            execute: async ({ service_id, service_name, professional_id, date }) => safeTool("list_slots", async () => {
                const cfg = await getBempConfig();
                const { effectiveUnitId } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                if (!effectiveUnitId)
                    throw new Error("ID da unidade não resolvido.");
                const { resolveServiceAssignment, getAvailableServiceAssignments, validateProfessionalServiceAssignment } = await import("@/lib/bemp/assignments.server");
                ...
                const slots = await bempFetch(url);
        ...
        create_appointment: tool({
            description: "Cria o agendamento na Bemp. Só chame após confirmação explícita do cliente. O 'end' deve ser o 'start' + duração do serviço em minutos.",
            inputSchema: z.object({
                salon_id: z.number().optional(),
                service_id: z.number(),
                professional_id: z.number().optional(),
            ...
            execute: async (input) => safeTool("create_appointment", async () => {
                const { effectiveUnitId } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                if (!effectiveUnitId)
                    throw new Error("ID da unidade não resolvido.");
        ...
        list_customer_appointments: tool({
            description: "Lista os agendamentos existentes de um cliente pelo telefone. Use antes de cancelar para achar o ID correto.",
            inputSchema: z.object({
                phone_country_code: z.string(),
                phone_area_code: z.string(),
                phone_number: z.string(),
            }),
            execute: async ({ phone_country_code, phone_area_code, phone_number }) => safeTool("list_customer_appointments", async () => {
        ...
        cancel_appointment: tool({
            description: "Cancela um agendamento existente na Bemp. Só chame após confirmação explícita do cliente sobre qual agendamento cancelar. O sistema registrará a tentativa de reagendamento em seguida.",
            inputSchema: z.object({
            ...
            execute: async ({ appointment_id, phone_country_code, phone_area_code, phone_number }) => safeTool("cancel_appointment", async () => {
        ...
        reschedule_appointment: tool({
            description: "Reagenda um agendamento existente na Bemp para uma nova data/hora, opcionalmente trocando serviço/unidade/profissional. Cria PRIMEIRO o novo agendamento e só depois cancela o antigo — assim o cliente nunca fica sem horário. Só chame após confirmação explícita do cliente.",
            inputSchema: z.object({
            ...
            execute: async (input) => safeTool("reschedule_appointment", async () => {
                if (sandbox) {
                    ...
                    try {
                        ...
                        const { effectiveUnitId } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                        await supabaseAdmin.from("reagendamentos_hist").insert({
                    ...
                    return {
                        ...
                        status: "simulated_rescheduled",
                        message: "Reagendamento SIMULADO (modo sandbox). Nada foi alterado na Bemp. (Confirmação por WhatsApp não é enviada em sandbox.)",
                ...
                if (!sandbox) {
                    try {
                        ...
                        console.error("[reschedule_appointment] novo criado mas cancelamento do antigo falhou:", oldCancelError);
                    }
                }
        ...
        list_subscription_plans: tool({
            description: "Lista os planos de assinatura cadastrados na Bemp (nome e resumo). Use quando o cliente perguntar sobre assinaturas, mensalidades, planos ou pacotes.",
            inputSchema: z.object({}),
            execute: async () => safeTool("list_subscription_plans", async () => {
                const cfg = await getBempConfig();
        ...
        get_subscription_plan: tool({
            description: "Retorna os detalhes completos de um plano de assinatura (descrição, benefícios, valores, condições).",
            inputSchema: z.object({ plan_id: z.number() }),
            execute: async ({ plan_id }) => safeTool("get_subscription_plan", async () => {
                const cfg = await getBempConfig();
                return await bempFetch(`${cfg.apiBase}/subscription_plans/${plan_id}`);
            }),
        }),
        lookup_customer: tool({
            description: "Verifica se um cliente já possui cadastro na Bemp pelo telefone. Retorna os dados quando existe, ou uma indicação de que precisa ser cadastrado.",
            inputSchema: z.object({
                phone_country_code: z.string(),
                phone_area_code: z.string(),
                phone_number: z.string(),
            }),
            execute: async ({ phone_country_code, phone_area_code, phone_number }) => safeTool("lookup_customer", async () => {
                const qs = new URLSearchParams({
                    phone_country_code,
                    phone_area_code,
                    phone_number,
                ...
                try {
                    const data = await bempFetch(`${BEMP_WEBHOOK_BASE}/whatsapp_customer?${qs.toString()}`);
        ...
        join_waiting_list: tool({
            description: "Adiciona o cliente à lista de espera para uma unidade e serviço específicos. Útil quando não há horários disponíveis no momento ou para o profissional preferido.",
            inputSchema: z.object({
                service_id: z.string(),
            ...
            execute: async (input) => safeTool("join_waiting_list", async () => {
                const { effectiveUnitId } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                if (!effectiveUnitId)
                    throw new Error("ID da unidade não resolvido.");
                const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                await supabaseAdmin
                    .from("crm_waiting_list")
                    .insert({
        ...
        get_customer_active_plans: tool({
            description: "Consulta no BEMP se o cliente possui plano de assinatura ATIVO (usando o telefone do WhatsApp). Chame SEMPRE que o cliente mencionar plano, benefício, assinatura ou 'quero usar meu plano'. Retorna os planos ativos com validade e saldo, e também os planos inválidos com o motivo.",
            inputSchema: z.object({
                phone_country_code: z.string(),
                phone_area_code: z.string(),
                phone_number: z.string(),
            }),
            execute: async ({ phone_country_code, phone_area_code, phone_number }) => safeTool("get_customer_active_plans", async () => {
                ...
                return {
                    ...
                    message: plans.length === 0 && res.inactivePlans.length > 0
        ...
        validate_subscription_phone: tool({
            description: "Valida o telefone cadastrado no plano de assinatura no BEMP. Retorna o cliente e os planos ativos. Se encontrar múltiplos planos, peça ao cliente para escolher um.",
            inputSchema: z.object({
                phone: z.string().describe("Número de telefone com DDD"),
            }),
            execute: async ({ phone }) => safeTool("validate_subscription_phone", async () => {
        ...
        resolve_subscription_service: tool({
            description: "Resolve, na unidade EFETIVA da conversa, qual serviço do BEMP corresponde ao plano do cliente (ex.: plano de manicure → 'Manicure Plano Beauty'). Chame antes de consultar profissionais/horários de um agendamento por plano. O backend resolve o service_id — nunca invente IDs.",
            inputSchema: z.object({
                plan_name: z.string().describe("Nome do plano retornado por get_customer_active_plans"),
            }),
            execute: async ({ plan_name }) => safeTool("resolve_subscription_service", async () => {
                const { effectiveUnitId } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                if (!effectiveUnitId)
                    throw new Error("ID da unidade não resolvido.");
                ...
                if (!res.success) {
                    return {
                        ...
                        message: res.code === "plan_not_mapped"
        ...
        create_subscription_appointment: tool({
            description: "Cria o agendamento usando o BENEFÍCIO do plano de assinatura. Use no lugar de create_appointment quando o atendimento for por plano. O backend revalida plano, saldo, unidade e resolve o service_id correto — informe apenas o nome do plano. Só chame após a confirmação explícita do resumo pelo cliente.",
            inputSchema: z.object({
                plan_name: z.string(),
                professional_id: z.number().optional(),
            ...
            execute: async (input) => safeTool("create_subscription_appointment", async () => {
                const { getCustomerActivePlans, resolveSubscriptionService, subscriptionAppointmentKey, getIdempotentSubscriptionResult, rememberSubscriptionResult, normalizeSubscriptionPlanName, } = await import("@/lib/bemp/subscriptions.server");
                ...
                const { data: convData } = await (await import("@/integrations/supabase/client.server")).supabaseAdmin
                    ...
                    .eq("phone", conversationKey || "")
                    .maybeSingle();
                const ctx = convData?.customer_context || {};
                let lookup;
                ...
                const plan = lookup.plans.find((p) => normalizeSubscriptionPlanName(p.name) === wanted) ??
                ...
                if (!plan) {
                    ...
                    return {
                        ...
                        message: "Seu plano foi localizado, mas não há utilização disponível no momento. Ofereça o agendamento como serviço comum ou atendimento humano. NUNCA diga que o benefício foi utilizado.",
                ...
                if (!resolved.success) {
                    ...
                    return {
                        success: false,
                        code: resolved.code,
                        message: "O serviço do plano não está disponível nesta unidade. Não substitua pelo serviço comum: informe a indisponibilidade ou ofereça transferência.",
                    };
                }
                ...
                const { validateProfessionalServiceAssignment, getProfessionalsForService } = await import("@/lib/bemp/assignments.server");
                ...
                try {
                    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                    await supabaseAdmin.from("agendamentos_notif").insert({
        ...
        register_subscription_lead: tool({
            description: "Registra o interesse do cliente em um plano de assinatura (cria o cadastro no nosso backend). Use SOMENTE após confirmação explícita do cliente. A equipe da unidade finaliza o pagamento e ativa a assinatura na Bemp.",
            inputSchema: z.object({
                plan_id: z.number(),
                plan_name: z.string(),
                name: z.string(),
            ...
            execute: async (input) => safeTool("register_subscription_lead", async () => {
                if (sandbox) {
                    return {
                        sandbox: true,
                        simulated: true,
                        ...
                        status: "simulated",
                        message: "Interesse em assinatura SIMULADO (modo sandbox). Nada foi gravado.",
                        lead: input,
        ...
        list_cross_sell_suggestions: tool({
            ...
            execute: async ({ salon_id, trigger_service_id, date, phone_country_code, phone_area_code, phone_number, }) => safeTool("list_cross_sell_suggestions", async () => {
                const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                const { effectiveUnitId } = await resolveEffectiveUnit({ conversationKey, agentUnitId: fallbackAgentUnitId });
                ...
                if (error)
                    throw new Error(error.message);
                const rules = (regras ?? []).filter((r) => !r.salon_id || r.salon_id === salonKey);
                if (rules.length === 0)
                    return { suggestions: [], reason: "sem_regras" };
                // Elegibilidade: já agendado no mesmo dia
                let sameDayServiceIds = new Set();
                try {
                    const qs = new URLSearchParams({
                        phone_country_code,
                        phone_area_code,
                    ...
                    const appts = await bempFetch(`${BEMP_WEBHOOK_BASE}/whatsapp_schedule?${qs.toString()}`);
                    const list = Array.isArray(appts)
                        ? appts
                        : Array.isArray(appts?.data)
                            ? appts.data
                            : [];
                    for (const a of list) {
                        const rec = a;
                        ...
                        if (!startStr.startsWith(date))
                            continue;
                        const sid = rec.service_id ?? rec.serviceId ?? rec.service?.id;
                        if (sid != null)
                            sameDayServiceIds.add(String(sid));
                    }
                }
                catch {
                ...
                for (const r of registros) {
                    if (r.status === "ofertado" || r.status === "aceito") {
                        countByService.set(r.suggested_service_id, (countByService.get(r.suggested_service_id) ?? 0) + 1);
                    }
                    if (r.status === "ofertado" && r.phone === phoneKey) {
                        countByCustomer.set(r.suggested_service_id, (countByCustomer.get(r.suggested_service_id) ?? 0) + 1);
                    }
                }
                const totalCustomerToday = Array.from(countByCustomer.values()).reduce((a, b) => a + b, 0);
                const eligible = [];
                const skipped = [];
                for (const r of rules) {
                    ...
                    if (r.limite_por_servico_dia != null &&
                    ...
                    if (r.limite_por_cliente_dia != null &&
                ...
                const conversaCap = eligible.reduce((min, e) => (e.limite_por_conversa != null ? Math.min(min, e.limite_por_conversa) : min), Number.MAX_SAFE_INTEGER);
                ...
                if (allSkipped.length > 0) {
                    try {
                        ...
                        await supabaseAdmin.from("sugestoes_registros").insert(allSkipped.map((s) => {
                ...
                return {
                    suggestions: capped,
                    skipped: allSkipped,
                    note: "Use list_services(salon_id) para obter valor e duração de cada suggested_service_id antes de oferecer.",
                };
            }),
        }),
        record_suggestion: tool({
            description: "Registra o resultado de uma sugestão de serviço complementar feita ao cliente (ofertado, aceito ou recusado). Use logo depois de oferecer e novamente quando o cliente responder.",
            inputSchema: z.object({
            ...
            execute: async (input) => safeTool("record_suggestion", async () => {
                const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                const { error } = await supabaseAdmin
                    .from("sugestoes_registros")
                    .insert({
                    ...
                    trigger_service_id: input.trigger_service_id != null ? String(input.trigger_service_id) : null,
            ...
            execute: async ({ phone_country_code, phone_area_code, phone_number }) => safeTool("check_subscription_balance", async () => {
                ...
                try {
                    ...
                    if (/404|not\s*found|não encontrado/i.test(message)) {
                        return {
                            found: false,
                            message: "Não encontramos cadastro na Bemp para esse telefone. Ofereça registrar uma consulta de saldo com a equipe usando register_balance_inquiry.",
                        };
                    }
                    throw err;
                }
                ...
                const planName = plan?.name ??
                    ...
                    customer?.plan_name ??
                    null;
                const planQuotaRaw = plan?.monthly_quota ??
                    plan?.visits_per_month ??
                    plan?.quota ??
                    plan?.limit ??
                    plan?.max_visits ??
                    null;
                const planQuota = typeof planQuotaRaw === "number"
                    ? planQuotaRaw
                ...
                const remaining = planQuota != null ? Math.max(planQuota - scheduledThisMonth, 0) : null;
                return {
                    found: true,
                    plan_name: planName,
                    ...
                    disclaimer: "A API pública da Bemp não expõe o saldo real do plano. Estes números são uma ESTIMATIVA baseada nos agendamentos futuros do mês. Ofereça ao cliente encaminhar para a equipe (use register_balance_inquiry) para confirmação oficial.",
                };
            }),
        }),
        register_balance_inquiry: tool({
            description: "Registra um pedido de verificação de saldo de visitas do plano de assinatura para a equipe humana confirmar na Bemp. Use quando o cliente quiser o número exato ou quando a estimativa não estiver disponível.",
            inputSchema: z.object({
                name: z.string(),
                phone_country_code: z.string(),
                phone_area_code: z.string(),
            ...
            execute: async (input) => safeTool("register_balance_inquiry", async () => {
                if (sandbox) {
                    return {
                        sandbox: true,
                        simulated: true,
                        id: `SIM-SALDO-${Date.now()}`,
                        message: "Pedido de verificação de saldo SIMULADO (sandbox). Nada foi gravado.",
                    };
                }
                const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
                const { data, error } = await supabaseAdmin
                ...
                return {
                    ok: true,
                    lead_id: data?.id,
                    message: "Pedido registrado. A equipe da unidade vai confirmar o saldo real na Bemp e retornar o contato.",
                };
            }),
        }),
        request_human_handoff: tool({
            description: "Registra que o cliente deseja falar com um atendente humano. Use quando o cliente pedir para falar com uma pessoa, quando reclamar de problema não resolvido, ou quando o assunto sair do escopo (reembolso, laudo, situação delicada). Confirme o motivo antes de chamar.",
            inputSchema: z.object({
            ...
            execute: async (input) => safeTool("request_human_handoff", async () => {
                if (sandbox) {
                    return {
                        sandbox: true,
                        simulated: true,
                ...
                return {
                    ok: true,
                    id: data?.id,
                    message: "Solicitação registrada. Um atendente humano vai retornar o contato em breve.",
                };
            }),
        }),
    };
...
import { ALLOW_SUBSCRIPTION_CPF_FALLBACK, SUBSCRIPTION_PRIMARY_LOOKUP, SUBSCRIPTION_MAX_PHONE_ATTEMPTS, SUBSCRIPTION_MESSAGES, LEGACY_CPF_CONTEXT_RESET, isBempTechnicalError, } from "@/lib/subscription-policy.server";
...
export function replacePromptVariables(prompt, values) {
    let out = prompt;
...
export function mandatoryOperationalRules(opts) {
    ...
    if (opts.unidadeId) {
        lines.push(`- A unidade de atendimento atual é: ${opts.unitName || `Unidade vinculada ID ${opts.unidadeId}`} (ID ${opts.unidadeId}).`, "- Se o cliente disser explicitamente que deseja agendar em OUTRA unidade, você deve iniciar o fluxo de transferência descrito nas regras acima.", "- Informe que o agendamento por este canal é realizado para a unidade atual, mas que você pode transferir a conversa se ele preferir.", "- É PROIBIDO perguntar a unidade no início se já houver uma unidade atual definida.");
    }
    if (opts.contactPhone) {
        lines.push("- O telefone do WhatsApp atual já é conhecido e não deve ser solicitado novamente para contatos comuns.", "- EXCEÇÃO OBRIGATÓRIA: quando a cliente mencionar plano, assinatura ou benefício e ainda não houver subscriptionPhoneValidated, pergunte qual é o telefone cadastrado na assinatura, pois ele pode ser diferente do número atual do WhatsApp.", "- Durante AWAITING_REGISTERED_PHONE, é permitido e obrigatório pedir o telefone cadastrado com DDD.");
    }
    if (opts.contactName)
        lines.push("- É PROIBIDO perguntar o nome do cliente: já é conhecido.");
    return lines.join("\n");
}
...
export function detectServiceCategory(message) {
    ...
    for (const [category, aliases] of Object.entries(SERVICE_CATEGORY_ALIASES)) {
        if (aliases.some(alias => normalized.includes(normalizeServiceSearchText(alias)))) {
            return { category: category, confidence: 1 };
        }
    }
    return null;
}
export function ensureMandatoryPromotionMessage(text, promotion) {
...
export function assembleSystemPrompt(basePrompt, opts) {
...
function sanitizeMessagesForModel(messages) {
    return messages.map((message) => ({
        ...message,
        parts: message.parts.map((part) => part.type === "text" ? { ...part, text: sanitizeCustomerText(part.text) } : part),
    }));
}
function envSandbox() {
...
export async function streamAgent(uiMessages, opts = {}) {
    ...
    try {
        ...
        if (!historyData) {
            ...
            await logEvent({
                ...
                status: "success",
                payload: { historyCount: historyData.messages?.length || 0 }
            });
        }
        ...
        try {
            ...
            const found = Array.isArray(list)
                ? list.find((s) => String(s?.id) === String(unidadeId))
                : null;
            if (found?.name || found?.nome)
                unitName = String(found.name || found.nome);
        }
        catch {
        ...
        for (const m of messagesArray) {
            if (m.id && !seenIds.has(m.id)) {
                seenIds.add(m.id);
                const text = Array.isArray(m.parts) ? m.parts.map((p) => p.text).join(" ").trim() : String(m.parts || "").trim();
                if (text.length > 0) {
        ...
        const { promptBlock: memoryBlock } = await loadMemoryForAgent({
            phone,
            instance,
            contactName: pushName || historyData?.contact_name || null,
        });
        const agentResult = await runAgent(historyMessages, {
            unidadeId: effectiveUnitId || undefined,
            unitName: currentUnitName || undefined,
            contactName: pushName || historyData?.contact_name,
            contactPhone: phone,
        ...
        if (mandatoryPromo && !historyData?.customer_context?.mechasPromotionPresented) {
        ...
        const cpfGuard = enforceNoCpfInSubscriptionFlow(reply, currentCustomerContext);
        if (cpfGuard.blocked) {
        ...
        await updateCustomerPipeline({
            ...
            customerName: pushName || historyData?.contact_name || undefined
        ...
        try {
            ...
            await learnFromInteraction({
                ...
                recentHistory: historyMessages.slice(-8).map((m) => ({
                    role: String(m.role),
                    text: Array.isArray(m.parts) ? m.parts.map((p) => p?.text ?? "").join(" ") : "",
                })),
        ...
        console.error(`[chat] ai_request_failed: conversationKey=${conversationKey}, unitId=${unidadeId}, code=${failure.code}, name=${info.name}, message=${info.message}`);
        if (info.stack)
            console.error(`[chat] ai_request_failed_stack:\n${info.stack}`);
        await logEvent({
            instance,
            messageId,
...
export async function runAgent(uiMessages, opts = {}) {
    const sandbox = opts.sandbox === true || envSandbox();
    let unitContext = "";
    if (opts.unidadeId) {
        unitContext = `\n\nUNIDADE: ID ${opts.unidadeId}${opts.unitName ? ` (${opts.unitName})` : ""}.`;
