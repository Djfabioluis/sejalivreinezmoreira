...
function buildTools(sandbox, forcedUnitId) {
    const base = {
        ...
        list_units_info: tool({
            description: "Use quando o cliente perguntar quantas unidades existem, quais são as unidades/lojas/endereços. Retorna as unidades ativas na Bemp com endereço. NÃO use para trocar a unidade do agendamento.",
        ...
        list_services: tool({
            ...
            execute: async ({ salon_id }) => safeTool("list_services", async () => {
                const cfg = await getBempConfig();
        ...
        list_professionals: tool({
            ...
            execute: async ({ salon_id, service_id }) => safeTool("list_professionals", async () => {
                const cfg = await getBempConfig();
                const targetUnitId = forcedUnitId || salon_id;
                if (!targetUnitId)
                    throw new Error("ID da unidade não fornecido.");
                return await bempFetch(`${cfg.apiBase}/salons/${targetUnitId}/services/${service_id}/professionals`);
            }),
        }),
        list_slots: tool({
            description: "Lista horários disponíveis. Passe professional_id apenas se o cliente escolheu um profissional específico.",
            inputSchema: z.object({
            ...
            execute: async ({ salon_id, service_id, professional_id, date }) => safeTool("list_slots", async () => {
                const cfg = await getBempConfig();
                const targetUnitId = forcedUnitId || salon_id;
                if (!targetUnitId)
                    throw new Error("ID da unidade não fornecido.");
        ...
        create_appointment: tool({
            ...
            execute: async (input) => safeTool("create_appointment", async () => {
                ...
                try {
                    ...
                    try {
                        const cfg = await getBempConfig();
                        const services = (await bempFetch(`${cfg.apiBase}/salons/${targetUnitId}/services`));
...
export function mandatoryOperationalRules(opts) {
    ...
    if (opts.unidadeId) {
        lines.push(`- A unidade de atendimento é FIXA: ${opts.unitName || `Unidade vinculada ID ${opts.unidadeId}`} (ID ${opts.unidadeId}).`, "- É PROIBIDO perguntar ou sugerir a troca de unidade para o agendamento. NUNCA pergunte \"qual dessas unidades você prefere?\" ou \"em qual unidade deseja ser atendido?\". Não chame list_salons (ela não está disponível).", "- EXCEÇÃO INFORMATIVA (OBRIGATÓRIA): se o cliente perguntar quantas unidades existem, quais são as unidades/lojas, os endereços das OUTRAS unidades ou os telefones, você DEVE responder com as informações completas de TODAS as unidades. Chame list_units_info e informe as unidades ATIVAS com o endereço de cada uma, uma por linha no formato \"• <Nome> — <Endereço>\". Em seguida lembre que o agendamento por este WhatsApp é feito na unidade <NOME DA UNIDADE FIXA>. NÃO pergunte qual delas o cliente prefere.", "- É PROIBIDO responder que \"o atendimento por aqui é exclusivo desta unidade\" como forma de NEGAR endereços, telefones ou a lista de unidades. A restrição de unidade vale apenas para o AGENDAMENTO, nunca para informações de localização e contato. Nunca interprete menção a outra unidade como uma mudança operacional.");
    }
