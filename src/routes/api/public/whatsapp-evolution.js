        ...
        handlers: {
            POST: async ({ request }) => {
                ...
                if (eventData.event === "connection.update") {
                    await processConnectionUpdate(payload).catch(err => logger.error("ASYNC_CONNECTION_UPDATE_ERROR", err.message, { traceId }));
                }
                    ...
                    await processMessageAck(payload).catch(err => logger.error("ASYNC_ACK_PROCESS_ERROR", err.message, { traceId }));
                }
                else {
